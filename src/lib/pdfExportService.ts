...
export class PDFExportService {
  ...
  static async exportAssetsToPDF(assets: Asset[], analytics: AssetAnalytics, fileName: string = 'assets-report.pdf') {
    ...
    const assetsData: RowInput[] = assets.map(asset => [
      ...
      `₹${(asset.quantity * asset.rate).toLocaleString()}`,
    ...
    autoTable(doc, {
      startY: currentY + 20,
      head: [['Name', 'Type', 'Lab', 'Qty', 'Rate', 'Total', 'Status', 'Created']],
      body: assetsData,
      theme: 'grid',
      headStyles: { fillColor: [59, 130, 246] },
      styles: { fontSize: 8 },
    ...
    Object.entries(analytics.costByLab).forEach(([lab, cost]) => {
      ...
      doc.text(`  ${lab}: ₹${cost.toLocaleString()}`, 25, yPos);
      yPos += 6;
    });
    
    // Footer
    doc.setFontSize(8);
    doc.text('© Asset Management System - Generated Report', 105, 290, { align: 'center' });
    
    doc.save(fileName);
  }
  
  static async exportIssuesToPDF(issues: AssetIssue[], analytics: IssueAnalytics, fileName: string = 'issues-report.pdf') {
    const doc = this.getDoc();
    ...
    const summaryData: RowInput[] = [
      ...
      ['Estimated Repair Cost', `₹${analytics.costAnalysis.estimatedRepairCost.toLocaleString()}`],
      ['Potential Replacement Cost', `₹${analytics.costAnalysis.replacementCost.toLocaleString()}`],
      ['Total Potential Cost', `₹${analytics.costAnalysis.totalPotentialCost.toLocaleString()}`]
    ];
    ...
    const issuesData: RowInput[] = issues.map(issue => [
      ...
      issue.issue_description.substring(0, 50) + (issue.issue_description.length > 50 ? '...' : ''),
      issue.status,
