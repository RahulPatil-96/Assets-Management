...
export const debugSignUp = async (email: string, password: string, userData: UserData) => {
  ...
  try {
    ...
    // console.error('❌ Connection failed:', _err);
  }
...
export const checkDatabaseTriggers = async () => {
  ...
  try {
    ...
    const { error: profilesError } = await supabase.from('user_profiles').select('count').limit(1);
    ...
    const { error: usersError } = await supabase.from('auth.users').select('count').limit(1);
    ...
    // console.error('❌ Database check failed:', _err);
  }
