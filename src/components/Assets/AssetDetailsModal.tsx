  ...
  useEffect(() => {
    ...
    const fetchCreatorName = async () => {
      ...
      try {
        ...
        // console.error('Error fetching creator name:', _error);
        setCreatorName('');
      } finally {
      ...
      <div className='bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto'>
        ...
        <div className='p-6 space-y-6'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div className='space-y-2'>
              ...
              <div className='space-y-1'>
                <p className='text-sm'>
                  <span className='font-medium'>Asset ID:</span> {asset.asset_id || 'Pending...'}
                </p>
                <p className='text-sm'>
                  <span className='font-medium'>Name:</span> {asset.name_of_supply}
                </p>
                <p className='text-sm'>
          ...
          <div className='space-y-2'>
            ...
            <div className='space-y-1'>
              <p className='text-sm'>
                <span className='font-medium'>Description:</span> {asset.description || 'N/A'}
              </p>
              <p className='text-sm'>
                <span className='font-medium'>Invoice Number:</span> {asset.invoice_number || 'N/A'}
              </p>
              <p className='text-sm'>
                <span className='font-medium'>Remarks:</span> {asset.remark || 'N/A'}
              </p>
            </div>
          </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div className='space-y-2'>
              ...
              <div className='space-y-1'>
                <p className='text-sm'>
                  ...
                  {loadingCreator ? 'Loading...' : creatorName}
                </p>
                <p className='text-sm'>
            ...
            <div className='space-y-2'>
              ...
              <div className='space-y-1'>
                {asset.approved_by && (
                  <p className='text-sm'>
                    <span className='font-medium'>HOD Approved By:</span>{' '}
                    {asset.approver?.name || ''}
                    {asset.approved_at && (
                ...
                {asset.approved_by_lab_incharge && (
                  <p className='text-sm'>
                    <span className='font-medium'>Lab Incharge Approved By:</span>{' '}
                    {asset.approver_lab_incharge?.name || ''}
