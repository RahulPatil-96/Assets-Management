...
const TransferForm: React.FC<TransferFormProps> = ({ onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState({
    asset_id: '',
    to_lab: '', // Placeholder to be replaced with dynamic lab selection
  });
        ...
        <form onSubmit={handleSubmit} className='p-6 space-y-4'>
          ...
          <div className='bg-blue-50 dark:bg-blue-900 p-4 rounded-lg'>
            <p className='text-sm text-blue-700 dark:text-blue-200'>
              ...
              <strong>{profile?.lab_id}</strong> to{' '}
              <strong>{formData.to_lab || '[Destination Lab]'}</strong>. The receiving lab assistant
              must confirm receipt to complete the transfer.
            </p>
          </div>
