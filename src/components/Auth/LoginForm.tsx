  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const { error: signUpError } = await signUp(email, password, { name, role, lab_id: labId });
      ...
      <div className="w-full max-w-md bg-white/10 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 overflow-hidden transform transition-all hover:scale-[1.01] duration-300">
        <div className="p-8 text-center">
          ...
          <p className="text-white/70 mb-8 text-center">
            {isLogin ? 'Sign in to access your AssetFlow dashboard' : 'Create a new AssetFlow account to get started'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {!isLogin && (
              <>
                ...
                <div className="flex flex-col text-left">
                  <label className="mb-1 text-white/80 text-sm font-medium" htmlFor="labId">Lab</label>
                  <div className="relative flex items-center">
                    <select
                      id="labId"
                      name="labId"
                      ...
                      className="w-full px-4 py-2 pr-10 rounded-md border border-white/20 bg-white/5 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 ease-in-out"
                    >
                      <option className="bg-gray-800 text-white" value="">Select a lab</option>
                      <option className="bg-gray-800 text-white" value="CSE-LAB-01">Computer Science Lab 01</option>
                      <option className="bg-gray-800 text-white" value="CSE-LAB-02">Computer Science Lab 02</option>
                      <option className="bg-gray-800 text-white" value="MECH-LAB-01">Mechanical Engineering Lab 01</option>
                      <option className="bg-gray-800 text-white" value="MECH-LAB-02">Mechanical Engineering Lab 02</option>
                      <option className="bg-gray-800 text-white" value="ECE-LAB-01">Electronics & Communication Lab 01</option>
                      <option className="bg-gray-800 text-white" value="ADMIN">Administration</option>
                    </select>
            ...
            <div className="flex justify-between items-center mt-4">
              <div className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <label htmlFor="rememberMe" className="text-white/60 text-sm">Remember Me</label>
              </div>
